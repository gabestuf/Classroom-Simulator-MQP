...
class Room {
  ...
  generateRoomElements() {
    ...
    for (let i = 0; i < this.RoomCfg.numBookshelves; i++) {
      let newCoord: Coordinate | null = this.findRandomEmptySpaceAlongEdge();
      if (newCoord === null) {
  ...
  getAdjacentTiles(coord: Coordinate): iTile[] {
    let adjacentTileList: iTile[] = [];
  ...
  findRandomEmptySpaceAlongEdge(): Coordinate | null {
    // same as findRandomEmptySpace but no coordinates against the wall
    let coordinateList: Coordinate[] = [];
    // we want to get all positions where there is floor
    for (const row of this.RoomArray) {
